...
**Points:** 40 Points
**By:** Ashmitha Paruchuri Balaji, Naman Chheda

---

## Lab Overview

This lab builds on our Lab 1 Yelp prototype. The application was containerized with Docker, orchestrated using Kubernetes, integrated with Kafka for asynchronous review and restaurant event processing, and deployed on AWS EKS with MongoDB as the persistent datastore. Redux Toolkit was integrated into the React frontend to manage authentication, restaurant, review, and favourites state. Performance characteristics were measured using Apache JMeter at 100–500 concurrent users.
...
Each Lab 1 service was containerized using Docker. Because all four Python services share the same codebase and dependencies, we used a single parametric Dockerfile (`lab2/docker/Dockerfile.service`) that takes the target service as a build argument. A separate Dockerfile (`lab2/docker/Dockerfile.frontend`) handles the React build + nginx multi-stage build. A third Dockerfile (`lab2/docker/Dockerfile.worker`) builds the Kafka consumer worker.
...
| Review Worker (Kafka)   | `:review-worker`    | python:3.12-slim    | n/a          |
| Frontend (React + nginx)| `:frontend`         | node:20 → nginx:alpine | 80        |

All images were tagged and pushed to **Amazon ECR** at `839408459700.dkr.ecr.us-west-2.amazonaws.com/yelp-lab2`.
...
> **[Insert screenshot: `docker images | grep yelp-lab2` showing all built images]**

> **[Insert screenshot: ECR Console showing all 6 images pushed to the `yelp-lab2` repository]**

---

## 1.2 Kubernetes Orchestration

The Dockerized services were deployed to an **Amazon EKS** cluster (`yelp-lab2`, region `us-west-2`) running 2 worker nodes of type **m7i-flex.large** (2 vCPU, 8 GiB RAM each — 16 GiB total cluster capacity). The control plane was provisioned using `eksctl`, and worker nodes use the AWS-managed node group.
...
| AWS Load Balancer Controller       | Provisions Network Load Balancers for `Service: LoadBalancer` |
...
- `deployment-user-service.yaml` (and 3 sibling deployment files for owner/restaurant/review)
...
- The gateway nginx proxies `/auth`, `/users`, `/favorites`, `/uploads`, `/notifications` → user-service; `/restaurants`, `/ai-assistant` → restaurant-service; `/reviews`, `/waitlist` → review-service; `/owner-dashboard` → owner-service.
- Frontend talks to the gateway over HTTP, which forwards to the appropriate backend.

> **[Insert screenshot: `kubectl get nodes` showing 2 nodes Ready]**

> **[Insert screenshot: `kubectl get pods -n yelp-lab2` showing all 15 pods in Running state]**
...
> **[Insert screenshot: AWS EC2 → Load Balancers showing both NLBs]**

> **[Insert screenshot: Frontend URL loaded in the browser showing the homepage with restaurant cards]**
...
The Kafka client wrapper (`lab2/python/kafka_client.py`) wraps `kafka-python` to provide a single `publish_event(topic, payload)` helper used by all producers, and a `consume()` generator used by the worker.
...
3. The API responds **immediately** (HTTP 202) with the job ID — the user is unblocked instantly even though writes happen later.
...
5. The frontend polls `GET /reviews/job/{job_id}` to display the final status to the user.
...
- **Horizontal scale:** more worker replicas can be added to consume in parallel from a partitioned topic.
...
> **[Insert screenshot: `kubectl get pods -n yelp-lab2` showing kafka, zookeeper, review-worker all Running]**
...
| `restaurant_views`   | `_id`, `restaurant_id`, `user_id` (nullable), `viewed_at` — used for trending analytics                                         |
| `notifications`      | `_id`, `user_id`, `type`, `title`, `body`, `is_read`, `created_at`                                                              |
...
> **[Insert screenshot: MongoDB Compass / mongosh showing the `yelp_lab2` database with all collections listed]**

> **[Insert screenshot: `users` collection — one document showing `password_hash`, `email`, `role`]**

> **[Insert screenshot: `sessions` collection — one document showing `token`, `expires_at`]**

> **[Insert screenshot: `restaurants` collection — one document showing `hours_of_operation` object, `cuisine_type`, `average_rating`]**

> **[Insert screenshot: `reviews` collection — documents with `owner_reply` and `photo_url` fields visible]**
...
- **Sessions are stored in MongoDB** as JWT-signed tokens. The `sessions` collection has a TTL index on `expires_at`, so expired tokens are automatically purged by Mongo.
- The JWT signing secret (`SECRET_KEY`) is injected from a Kubernetes ConfigMap into every backend service so all microservices verify the same token.

> **[Insert screenshot: bcrypt-hashed password stored in users collection (showing `$2b$12$...` format)]**
...
1. User submits login form → component dispatches the `login` thunk.
...
### Example: Review Flow with Optimistic State
...
4. Frontend polls `GET /reviews/job/{id}`; on completion, the `upsertReviewInState` reducer adds the review to `byRestaurant[restaurantId]` so the UI updates without re-fetching.
...
- **Redux frontend:** `frontend/src/store/`, `frontend/src/store/slices/`, slice integration across `pages/` and `components/`
- **JMeter test plans + results:** `lab2/jmeter/yelp_load_test.jmx`, `lab2/jmeter/results_*.jtl`, HTML reports
- **README:** updated with Lab 2 setup instructions (`lab2/scripts/install_alb_controller.sh`, `lab2/scripts/demo_setup.sh`)
- **Tests:** `lab2/tests/test_*.py` covering auth, users, restaurants, reviews, favourites, AI assistant, owner dashboard, hours, waitlist, and notifications (203 passing tests)

**Repository link:** https://github.com/Devdatta1999/Yelp_Prototype
...
> **[Insert screenshot: Restaurant detail page showing reviews, hours, AI sidebar]**

> **[Insert screenshot: AI sidebar showing personalized recommendations matching the user's saved preferences (cuisine, dietary, ambiance)]**
...
Beyond the core Lab 2 requirements, the following features were added (all live on AWS, all tested via 203 passing pytest tests under `lab2/tests/`):

1. **AI Restaurant Assistant** — GROQ LLM (llama3-8b-8192) integration with rule-based intent extraction (cuisine, city, dietary, ambiance, hours), preference-aware ranking, and per-user conversation memory persisted in MongoDB.
2. **Search autocomplete** — `GET /restaurants/autocomplete?q=...` returns top-8 matches by name/cuisine/city.
3. **Trending restaurants** — `GET /restaurants/trending?days=7` aggregates view counts and recent reviews into a trending score.
...
5. **Owner reply to reviews** — `POST /reviews/{id}/reply` lets a verified owner respond to reviews of their own restaurants.
6. **Review photo upload** — `POST /reviews/{id}/photo` accepts JPG/PNG/WebP up to 10 MB.
7. **Waitlist / virtual queue** — `POST /waitlist/{restaurant_id}` to join, `GET /waitlist/{restaurant_id}/status` to check position; owners can call guests via `POST /waitlist/{restaurant_id}/notify/{user_id}`.
8. **Notifications** — In-app notification system (`/notifications`) with unread count badge and read/unread toggling, fired on new reviews, owner replies, and waitlist events.
